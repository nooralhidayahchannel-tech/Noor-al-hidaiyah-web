...
export default function SurahPage() {
  ...
  const [contentMode, setContentMode] = useState<"none" | "tafsir" | "translation" | "both">("none");
  ...
  useEffect(() => {
                ...
                number,
                    { translation: string; tafsir: string }
                      > = {};
                        ...
                        for (const verse of (Array.isArray(contentJson) ? contentJson : (contentJson.verses ?? []))) {
                            contentMap[verse.verseNumber] = {
                                  translation: verse.translations?.[0]?.text ?? "",
                                        tafsir: verse.tafsirs?.[0]?.text ?? "",
                                                ...
                                                setContentData(Object.values(contentMap));
                                                }
    ...
    loadSurah();
    fetch(`/api/quran-content/${number}`).then(r => r.json()).then(data => setContentData(data.verses ?? [])).catch(() => setContentData([]));
  }, []);
            ...
            {mushafView ? (
              ...
              ayahs.map((ayah, index) => (
                ...
                >
                  <p>{ayah}</p>
              {contentMode !== "none" && (
                <div className="quran-content-extra">
                  {(contentMode === "translation" || contentMode === "both") && (
                    <div className="quran-translation">
                      <strong>الترجمة:</strong>
                      <span>{contentData[index]?.translation ?? "لا توجد ترجمة"}</span>
                    </div>
                  )}
                  {(contentMode === "tafsir" || contentMode === "both") && (
                    <div className="quran-tafsir">
                      <strong>التفسير:</strong>
                      <span>{contentData[index]?.tafsir ?? "لا يوجد تفسير"}</span>
                    </div>
                  )}
                </div>
              )}
